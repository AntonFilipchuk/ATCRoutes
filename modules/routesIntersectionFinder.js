...
export function findIntersectionBetween2Routes(route1, route2) {
  ...
  if (route1.name === route2.name) {
    console.error(
      `Trying to check intersections for the same route ${route1.name}!`
    );
    return undefined;
  }
  ...
  if (pointsRoute1Names.length == 0 || pointsRoute2Names.length == 0) {
    console.error(
      `Can't calculate intersections, no points for one or more route!`
    );
    return undefined;
  }
...
export function findIntersectionsBetweenAllRoutes(routes, routeToTest) {
  const intersectionsArray = [];
  routes.forEach((route) => {
    const intersections = findIntersectionBetween2Routes(routeToTest, route);
